...
/**
 * Get leads by minimum score (for prioritizing)
 */
export async function getLeadsByScore(minScore: number): Promise<ApiResponse<Lead[]>> {
  try {
    const leadsRef = collection(db, LEADS_COLLECTION);
    const q = query(
      leadsRef, 
      where('websiteScore', '>=', minScore),
    ...
    console.error('❌ Error getting leads by score:', error);
    return { success: false, error: error.message };
  }
}
