...
export async function generateAndDeploySite(
  ...
  if (!vercelProjectId) throw new Error("Missing VERCEL_PROJECT_ID");
...
Do NOT include markdown or explanations.`,
  ...
  let siteFiles: Record<string, string>;
  try {
  ...
  for (const file of requiredFiles) {
    if (!siteFiles[file]) throw new Error(`Missing ${file} in model output`);
